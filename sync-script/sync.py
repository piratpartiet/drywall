...
def updates_from_new_to_old():
    ...
    updated_rows = sql(new_conn, "select updated_at, coalesce(first_name, '')||' '||coalesce(last_name, ''),coalesce(date_birth,date '1900-01-01'+(year_birth-1900)*interval '1 year'),address,zip,email,municipality,county,(select min(payment_date) from payment where user_id=login_user.id and purpose=1), member_number  from login_user join member on user_id=login_user.id where exists (select * from payment where user_id=login_user.id and purpose=1 and payment_date>date_trunc(now(), 'year') and updated_at>%s", [last_run])
    for row in updated_rows:
        sql(old_conn, "update members set updated_at=%s. navn=%s, fdato=%s, adresse=%s, postnummer=%s, epost=%s, kommune=%s, fylke=%s, innmeldt=%s where mnr=%s", params=row)
        sql(old_conn, "update members set poststed=(select post.poststed from post where post.postnummer||''=members.postnummer) where created_at=now()") ## will this work?
    sql(old_conn, "insert into member_sync (op, started, finished, num_rows) values ('upd_new2old', now(), %s, %s)", params=[datetime.datetime.now(), len(updated_rows)])
    new_conn.commit()
    old_conn.commit()
